...
function resolveClubDisplayName(name = "", knownClubs = state.clubsRaw || state.clubs || []) {
  ...
  if (!raw) return "";
  const simplified = simplifyClubKey(raw);
  const canonical = CLUB_CANONICAL_NAMES[simplified];
  if (canonical) return canonical;
  const stripped = stripReserveClubSuffix(raw);
  const strippedKey = normalizeClubName(stripped);
  const known = knownClubs.find((club) => normalizeClubName(club) === strippedKey);
  return known ? (CLUB_CANONICAL_NAMES[simplifyClubKey(known)] || stripReserveClubSuffix(known)) : stripped || raw;
}
...
async function boot() {
  ...
  try {
    ...
    fillSelects(false);
    renderInitial();
    state.bootTimings.initialMs = Math.round(performance.now() - started);
    window.__fanRadarInitialBootMs = state.bootTimings.initialMs;
...
async function hydrateFullData() {
  ...
  try {
    ...
    fillSelects(true);
    renderAll(false);
    state.bootTimings.fullHydrationMs = Math.round(performance.now() - started);
...
function wireUi() {
  ...
  $("resetMatches").addEventListener("click", () => { state.activeClub = ""; state.prioritizeClubLens = false; $("hypeClubSelect").value = ""; state.playerSort = { key: "overall", dir: "desc" }; applyClubTheme(); renderMatches(); renderPlanner(); });
...
function fillSelects(full = state.fullHydrated) {
  ...
  const clubNames = canonicalClubList(state.clubs, state.clubsRaw);
  state.clubs = clubNames;
...
function updateControlVisibility() {
  ...
  $("showAllPlanner").classList.toggle("hidden", !["plannerTeam", "plannerDate", "plannerStage", "watchabilityFilter", "plannerSearch"].some((id) => $(id)?.value));
}
...
function matchCard(m) {
  ...
  const supporter = state.activeClub ? `<span class="component-pill club-pill"><b>Supporter Lens</b>${fmt(m.hype.supporterScore)}</span>` : "";
...
function tooltipText(m) {
  const c = m.hype.components;
  const supporter = state.activeClub ? ` | Supporter lens ${fmt(m.hype.supporterScore ?? supporterScore(m.hype))}` : "";
  return `Team quality ${fmt(c.country)}/30 | Player pull ${fmt(c.stars)}/25 | Closeness ${fmt(c.balance)}/20 | Story ${fmt(c.story)}/15 | Stage ${fmt(c.stage)}/10 | Club relevance ${clubRelevanceText(m.hype)}${supporter}`;
}
